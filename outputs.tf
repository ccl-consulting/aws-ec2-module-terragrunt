...
output "existing_ssm_endpoint_ids" {
  ...
  value       = var.create_vpc_endpoints && var.check_for_existing_vpc_endpoints ? data.aws_vpc_endpoints.existing_ssm[0].ids : []
}

output "existing_ec2messages_endpoint_ids" {
  description = "IDs of existing EC2Messages VPC endpoints found in the VPC"
  value       = var.create_vpc_endpoints && var.check_for_existing_vpc_endpoints ? data.aws_vpc_endpoints.existing_ec2messages[0].ids : []
}

output "existing_ssmmessages_endpoint_ids" {
  description = "IDs of existing SSMMessages VPC endpoints found in the VPC"
  value       = var.create_vpc_endpoints && var.check_for_existing_vpc_endpoints ? data.aws_vpc_endpoints.existing_ssmmessages[0].ids : []
}
...
output "created_ssm_endpoint_id" {
  ...
  value = var.create_vpc_endpoints && (
    var.check_for_existing_vpc_endpoints ? length(data.aws_vpc_endpoints.existing_ssm[0].ids) == 0 : true
...
output "created_ec2messages_endpoint_id" {
  ...
  value = var.create_vpc_endpoints && (
    var.check_for_existing_vpc_endpoints ? length(data.aws_vpc_endpoints.existing_ec2messages[0].ids) == 0 : true
...
output "created_ssmmessages_endpoint_id" {
  ...
  value = var.create_vpc_endpoints && (
    var.check_for_existing_vpc_endpoints ? length(data.aws_vpc_endpoints.existing_ssmmessages[0].ids) == 0 : true
...
output "vpc_endpoints_reused" {
  ...
  value = var.create_vpc_endpoints && var.check_for_existing_vpc_endpoints ? (
    length(data.aws_vpc_endpoints.existing_ssm[0].ids) > 0 ||
    length(data.aws_vpc_endpoints.existing_ec2messages[0].ids) > 0 ||
    length(data.aws_vpc_endpoints.existing_ssmmessages[0].ids) > 0
  ) : false
}
